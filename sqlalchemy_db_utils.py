...
class Interaction(Base):
    ...
    memory_interactions = relationship("MemoryInteraction", back_populates="interaction")
...
class DeepRecallORM:
    ...
    def get_session(self, session_id: str) -> Optional[Session]:
        ...
        """
        with self.get_session() as session:
            result = session.query(Session).filter(Session.id == session_id).first()
            return result
    ...
    def get_embedding_model(self) -> Optional[EmbeddingModel]:
        ...
        """
        with self.get_session() as session:
            model = session.query(EmbeddingModel).filter(EmbeddingModel.name == self.embedding_model_name).first()
    ...
    def update_memory(self, memory_id: str, **kwargs) -> Optional[Memory]:
        ...
        with self.get_session() as session:
            ...
            if 'text' in kwargs and self.embedding_model:
                try:
                    kwargs['embedding'] = self.embedding_model.encode(kwargs['text']).tolist()
                except Exception as e:
                    logger.warning(f"Failed to generate embedding: {e}")
        ...
        with self.get_session() as session:
            ...
            if filters:
                ...
                if 'session_id' in filters:
                    memories = memories.filter(Memory.session_id == filters['session_id'])
    ...
    def get_session_interactions(self, session_id: str) -> List[Interaction]:
        ...
        """
        with self.get_session() as session:
            interactions = session.query(Interaction).filter(Interaction.session_id == session_id).order_by(Interaction.created_at).all()
            return interactions
    
    def update_interaction(self, interaction_id: str, **kwargs) -> Optional[Interaction]:
        """
    ...
    def get_interaction_feedback(self, interaction_id: str) -> List[Feedback]:
        ...
        """
        with self.get_session() as session:
            feedback = session.query(Feedback).filter(Feedback.interaction_id == interaction_id).all()
            return feedback
