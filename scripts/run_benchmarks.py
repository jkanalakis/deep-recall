...
async def run_embedding_benchmarks(output_dir: str):
    ...
    benchmark.save_results(sts_results, os.path.join(output_dir, "sts_benchmark_results.json"))
    ...
    retrieval_results = benchmark.evaluate_retrieval_accuracy(queries, documents, ground_truth)
    benchmark.save_results(retrieval_results, os.path.join(output_dir, "retrieval_benchmark_results.json"))
    
    logger.info("Embedding benchmarks completed")


async def run_inference_benchmarks(output_dir: str, api_url: str, auth_token: str = None):
    """
    ...
    latency_results = await benchmark.benchmark_latency(prompts, batch_size=1, num_runs=5)
    benchmark.save_results(latency_results, os.path.join(output_dir, "latency_benchmark_results.json"))
    ...
    benchmark.save_results(throughput_results, os.path.join(output_dir, "throughput_benchmark_results.json"))
...
def generate_report(output_dir: str):
    ...
    if "sts_benchmark_results.json" in results:
        ...
        report["summary"]["embedding_quality"]["sts_correlation"] = sts_results.get("correlation", 0)
    
    if "retrieval_benchmark_results.json" in results:
        retrieval_results = results["retrieval_benchmark_results.json"][-1]  # Get the latest result
        report["summary"]["embedding_quality"]["retrieval_f1"] = retrieval_results.get("f1@5", 0)
    
    if "latency_benchmark_results.json" in results:
        latency_results = results["latency_benchmark_results.json"][-1]  # Get the latest result
        report["summary"]["inference_performance"]["avg_latency_ms"] = latency_results.get("avg_latency", 0) * 1000
        report["summary"]["inference_performance"]["p95_latency_ms"] = latency_results.get("p95_latency", 0) * 1000
    
    if "throughput_benchmark_results.json" in results:
        throughput_results = results["throughput_benchmark_results.json"][-1]  # Get the latest result
        report["summary"]["inference_performance"]["requests_per_second"] = throughput_results.get("requests_per_second", 0)
    ...
    with open(os.path.join(output_dir, "benchmark_report.md"), 'w') as f:
        ...
        f.write("|--------|-------|\n")
        f.write(f"| STS Correlation | {report['summary']['embedding_quality'].get('sts_correlation', 0):.4f} |\n")
        f.write(f"| Retrieval F1@5 | {report['summary']['embedding_quality'].get('retrieval_f1', 0):.4f} |\n")
        
        f.write("\n### Inference Performance\n\n")
        f.write("| Metric | Value |\n")
        f.write("|--------|-------|\n")
        f.write(f"| Average Latency | {report['summary']['inference_performance'].get('avg_latency_ms', 0):.2f} ms |\n")
        f.write(f"| P95 Latency | {report['summary']['inference_performance'].get('p95_latency_ms', 0):.2f} ms |\n")
        f.write(f"| Requests per Second | {report['summary']['inference_performance'].get('requests_per_second', 0):.2f} |\n")
        
        f.write("\n## Detailed Results\n\n")
        f.write("Detailed results are available in the JSON files in this directory.\n")
    
    logger.info(f"Benchmark report generated: {os.path.join(output_dir, 'benchmark_report.md')}")
...
async def main():
    ...
    parser.add_argument("--api-url", type=str, default="http://localhost:8000/v1/completions", help="URL of the inference API")
    parser.add_argument("--auth-token", type=str, help="Authentication token for the inference API")
    parser.add_argument("--skip-embedding", action="store_true", help="Skip embedding benchmarks")
    parser.add_argument("--skip-inference", action="store_true", help="Skip inference benchmarks")
