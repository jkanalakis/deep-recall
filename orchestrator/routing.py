...
class RequestRouter:
    ...
    def __init__(
        ...
        self.context_aggregator = context_aggregator or ContextAggregator(max_memories=max_memories)
    ...
    async def route_inference_request(
        ...
        if session_id:
            ...
            try:
                if self.memory_client:
                    session_metadata = await self.memory_client.get_session_metadata(session_id)
                    if session_metadata:
        ...
        if use_memory and self.memory_client:
            try:
                ...
                logger.info(f"Retrieved {len(memories)} memories in {memory_retrieval_time:.3f}s for user {user_id}")
            except Exception as e:
        ...
        logger.debug(f"Context aggregation completed in {context_aggregation_time:.3f}s")
            ...
            try:
                if prompt_format == "chat" and "chat_messages" in formatted_context:
                    ...
                    response = await self.inference_client.generate_chat_response(
                        messages=formatted_context["chat_messages"],
                        **inference_params
                    )
                else:
                    # Use text format
                    response = await self.inference_client.generate_response(
                        prompt=formatted_context["text_prompt"],
                        **inference_params
                    )
        ...
        enhanced_response = {
            **response,
            "context_metadata": {
                ...
                "context_aggregation_time": context_aggregation_time,
                "inference_time": asyncio.get_event_loop().time() - inference_start_time,
                "total_time": memory_retrieval_time + context_aggregation_time + 
        ...
        if request_data.get("include_memory_ids", False) and memories:
            enhanced_response["memory_ids"] = [
                m.get("id", "unknown") for m in memories
            ]
