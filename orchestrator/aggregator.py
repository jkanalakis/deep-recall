...
class ContextAggregator:
    ...
    async def aggregate_context(
        ...
        context = {
            ...
            "metadata": {
                "memory_sources": [m.get("source", "unknown") for m in selected_memories],
                "memory_timestamps": [m.get("timestamp", "") for m in selected_memories],
            }
        }
    ...
    def _prioritize_memories(
        ...
        for memory in memories:
            ...
            if session_context and session_context.get("session_id"):
                if memory.get("metadata", {}).get("session_id") == session_context.get("session_id"):
                    session_boost = 0.1
        ...
        logger.info(f"Selected {len(selected_memories)} memories from {len(memories)} candidates")
    ...
    async def _check_and_summarize(
        ...
        char_limit = int(len(formatted_memories) * char_ratio * 0.8)  # 0.8 for safety margin
        truncated_memories = formatted_memories[:char_limit] + "...\n\n[Note: Some memories were truncated]"
    ...
    def _create_system_instruction(
        ...
        if session_context:
            if "topic" in session_context:
                instruction += f"The current conversation is about: {session_context['topic']}. "
                
            if "previous_interactions" in session_context:
                count = session_context["previous_interactions"]
                instruction += f"This is interaction #{count+1} in the current session. "
    ...
    def format_for_llm(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "token_estimate": self._estimate_tokens(text_prompt) if self.tokenizer else None
        }
