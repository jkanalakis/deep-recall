...
class InferenceRequest(BaseModel):
    ...
    memory_filters: Dict[str, Any] = Field(
        {}, description="Filters for memory retrieval"
    )
    store_interaction: bool = Field(
        True, description="Whether to store this interaction"
    )
    include_memory_ids: bool = Field(
        False, description="Whether to return memory IDs in response"
    )
...
class InferenceResponse(BaseModel):
    ...
    context_metadata: Dict[str, Any] = Field(
        {}, description="Metadata about context used"
    )
...
class FeedbackRequest(BaseModel):
    ...
    interaction_id: str = Field(
        ..., description="ID of the interaction to give feedback on"
    )
...
class ApiGateway:
    ...
    def _setup_routes(self):
        ...
        async def inference(
            request: InferenceRequest, user_info=Depends(self._get_user_info)
        ):
            ...
            try:
                ...
                response = await self.router.route_inference_request(
                    request_data=request.dict(), user_id=user_id, session_id=session_id
                )
        ...
        async def store_feedback(
            request: FeedbackRequest, user_info=Depends(self._get_user_info)
        ):
            ...
            try:
                ...
                result = await self.router.store_user_feedback(
                    feedback_data=request.dict(), user_id=user_id
                )
        ...
        async def store_memory(
            request: StoreMemoryRequest, user_info=Depends(self._get_user_info)
        ):
            """Store a new memory."""
            try:
                user_id = user_info["user_id"]
        ...
        async def get_memories(
            ...
            try:
                ...
                memories = await self.router.memory_client.get_memories(
                    query=query or "", k=k, user_id=user_id, filters=filters
                )
