...
class FaissVectorDB(VectorDB):
    ...
    def _create_index(self):
        ...
        if self.index_type == "flat":
            ...
            index = faiss.IndexHNSWFlat(self.dimension, 32, metric_type)  # 32 is M parameter in HNSW
            return index
    ...
    def search(self, 
        ...
        if k == 0:
            ...
            return np.zeros((n_queries, 0), dtype=np.float32), np.zeros((n_queries, 0), dtype=np.int64)
    ...
    def save(self, path: str) -> bool:
        ...
        try:
            ...
            metadata = {
                ...
                "id_map": {str(k): v for k, v in self.id_map.items()},  # Convert keys to strings for JSON
    ...
    def load(self, path: str) -> bool:
        ...
        try:
            ...
            self.id_map = {int(k): v for k, v in metadata.get("id_map", {}).items()}  # Convert keys back to ints
    ...
    def optimize_index(self) -> bool:
        ...
        try:
            ...
            if self.index_type == "flat" and self.get_vector_count() > 10000:
                ...
                quantizer = faiss.IndexFlatL2(self.dimension)
                if self.metric == "ip":
                    new_index = faiss.IndexIVFFlat(quantizer, self.dimension, self.nlist, faiss.METRIC_INNER_PRODUCT)
                else:
                    new_index = faiss.IndexIVFFlat(quantizer, self.dimension, self.nlist, faiss.METRIC_L2)
                ...
                if self.get_vector_count() > 0:
                    ...
                    vectors = np.zeros((self.get_vector_count(), self.dimension), dtype=np.float32)
                    for i in range(self.get_vector_count()):
                        vector = faiss.vector_float_to_array(faiss.extract_index_vector(self.index, i))
                        vectors[i] = vector
