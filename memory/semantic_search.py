...
class SemanticSearch:
    ...
    def search(
        ...
        if not self.embedding_model:
            raise ValueError("No embedding model available. Either provide one during initialization or use search_with_embedding.")
    ...
    def search_with_embedding(
        ...
        sim_threshold = threshold if threshold is not None else self.default_similarity_threshold
        sim_metric = metric if metric is not None else self.default_similarity_metric
        items_per_page = min(items_per_page or self.max_results_per_page, self.max_results_per_page)
    ...
    def _recompute_similarities(
        ...
        for vec in vectors:
            if metric == "cosine":
                ...
                similarity = np.dot(query_embedding, vec) / (norm_q * norm_v) if norm_q * norm_v > 0 else 0
    ...
    def hybrid_search(
        ...
        for memory_id, text in self.memory_store.text_data.items():
            if filter_metadata:
                ...
                if memory_id not in self.memory_store.metadata or not self._matches_filter(
        ...
        keyword_results = [
            {"id": memory_id, "text": self.memory_store.text_data[memory_id], 
             "similarity": score, "metadata": self.memory_store.metadata.get(memory_id, {})}
            for memory_id, score in sorted(keyword_scores.items(), key=lambda x: x[1], reverse=True)[:k*2]
        ]
        ...
        for result in keyword_results:
            ...
            if memory_id in combined_scores:
                combined_scores[memory_id]["score"] += result["similarity"] * keyword_weight
            else:
                combined_scores[memory_id] = {
