...
async def retrieve_memories(
    user_id: str, query: str, max_results: int = 5
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Error retrieving memories: {response.status_code} - {response.text}"
                )
                return None
    ...
    for memory in memories:
        ...
        try:
            ...
            if (
                ...
                and "role" in memory_data
                and "content" in memory_data
            ):
        ...
        if "query" in memory.metadata and "response" in memory.metadata:
            ...
            context.append({"role": "user", "content": memory.metadata["query"]})
            context.append(
                {"role": "assistant", "content": memory.metadata["response"]}
            )
        else:
...
async def get_context_for_prompt(
    user_id: str, prompt: str, max_memories: int = 5
) -> List[Dict[str, str]]:
    """
