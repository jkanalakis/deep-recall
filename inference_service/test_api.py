        ...
        if response.status_code == 200:
            ...
            if result["available"]:
                ...
                for i, device in enumerate(result["devices"]):
                    ...
                    print(f"    Memory: {device['free_memory_gb']:.2f} GB free / {device['total_memory_gb']:.2f} GB total")
            else:
...
async def test_load_model_with_optimizations(model_name: str, quantization: str = None, parallel_mode: str = None):
...
async def test_batch_completion_performance(prompts: List[str], model: Optional[str] = None):
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        ...
        if response.status_code == 200:
            ...
            if gpu_response.status_code == 200:
                gpu_data = gpu_response.json()
                if gpu_data["available"]:
                    print(f"Final GPU Memory Usage: {gpu_data['used_memory_gb']:.2f} GB")
        else:
            print(f"Error: {response.status_code}")
            print(response.text)
...
async def main():
    ...
    parser.add_argument("--gpu-status", action="store_true", help="Test the GPU status endpoint")
    parser.add_argument("--load-optimized", action="store_true", help="Test loading a model with optimizations")
    parser.add_argument("--quantization", help="Quantization mode for model loading (none, int8, int4, gptq, awq)")
    parser.add_argument("--parallel", help="Model parallelism mode (none, tensor, pipeline, expert)")
    parser.add_argument("--clear-gpu", action="store_true", help="Test clearing GPU memory")
    parser.add_argument("--offload", action="store_true", help="Test offloading a model to CPU")
    parser.add_argument("--reload", action="store_true", help="Test reloading a model to GPU")
    parser.add_argument("--performance", action="store_true", help="Test batch performance with detailed metrics")
    ...
    if args.health or not any([args.health, args.models, args.completion, args.batch, args.personalized, 
                             args.gpu_status, args.load_optimized, args.clear_gpu, args.offload, 
                             args.reload, args.performance]):
        await test_health()
    ...
    if args.load_optimized:
        ...
        await test_load_model_with_optimizations(model_name, args.quantization, args.parallel)
