...
ENABLE_BATCH_PREFETCH = (
    os.environ.get("ENABLE_BATCH_PREFETCH", "false").lower() == "true"
)
ENABLE_DYNAMIC_BATCHING = (
    os.environ.get("ENABLE_DYNAMIC_BATCHING", "true").lower() == "true"
)
...
class BatchProcessor:
    ...
    def __init__(
        ...
        logger.info(
            f"Batch processor initialized with max_batch_size={max_batch_size}, "
        ...
        with self.lock:
            ...
            for model_name, batch in self.batches.items():
                ...
                if (
                    ...
                    or waiting_time_ms >= self.max_waiting_time_ms
                ):
    ...
    async def _process_batch(self, model_name: str, batch: List[InferenceRequest]):
        ...
        try:
            ...
            contexts = (
                [req.context for req in batch]
                if all(req.context is not None for req in batch)
                else None
            )
            ...
            try:
                        ...
                        request.future.set_result(
                            InferenceResponse(
                                ...
                                usage={
                                    ...
                                    "total_tokens": req_prompt_tokens
                                    + req_completion_tokens,
                                },
                                model=model_name,
                                inference_time=per_request_time
                ...
                logger.info(
                    f"Processed batch of {len(batch)} requests for model {model_name} "
                    f"in {batch_time:.2f}ms ({batch_time / len(batch):.2f}ms per request)"
                )

            except Exception as e:
                logger.error(
                    f"Error processing batch for model {model_name}: {str(e)}",
                    exc_info=True,
                )
