...
# Queue metrics
QUEUE_LENGTH = Gauge(
    "inference_queue_length", "Number of requests in the inference queue"
)
...
TOKEN_GENERATION_SPEED = Gauge(
    "tokens_per_second", "Token generation speed (tokens/sec)"
)
PROMPT_TOKEN_COUNT = Counter(
    "prompt_tokens_total", "Total number of prompt tokens processed"
)
COMPLETION_TOKEN_COUNT = Counter(
    "completion_tokens_total", "Total number of completion tokens generated"
)

# Resource utilization metrics
GPU_UTILIZATION = Gauge(
    "nvidia_gpu_utilization", "NVIDIA GPU utilization percentage", ["gpu_id"]
)
GPU_MEMORY_USAGE = Gauge(
    "nvidia_gpu_memory_used_bytes", "NVIDIA GPU memory used in bytes", ["gpu_id"]
)
MODEL_MEMORY_USAGE = Gauge("model_memory_bytes", "Memory used by model in bytes")
MEMORY_PER_REQUEST = Gauge(
    "memory_per_request_mb", "Average memory usage per request in MB"
)
...
BATCH_SIZE = Histogram(
    "batch_size", "Batch sizes used for inference", buckets=[1, 2, 4, 8, 16, 32]
)
REQUEST_RATE = Gauge("requests_per_second", "Number of requests per second")

# Error metrics
ERROR_COUNTER = Counter(
    "inference_errors_total", "Total number of inference errors", ["error_type"]
)
...
class MetricsExporter:
    ...
    def start_update_thread(self):
        ...
        self.update_thread = threading.Thread(
            target=self._update_resource_metrics, daemon=True
        )
