...
class DeepSeekR1Model:
    def __init__(self, 
        ...
        self.quantization = quantization if isinstance(quantization, QuantizationMode) else QuantizationMode(quantization)
        self.parallel_mode = parallel_mode if isinstance(parallel_mode, ParallelMode) else ParallelMode(parallel_mode)
        ...
        logger.info(f"DeepSeek R1 model loaded with optimal batch size: {self.optimal_batch_size}")
    ...
    def batch_generate_replies(self, 
                              prompts: List[str],
                              max_new_tokens: int = 1024,
                              temperature: float = 0.7,
                              top_p: float = 0.9,
                              contexts: Optional[List[List[Dict[str, str]]]] = None) -> List[str]:
        """
    ...
    async def generate_stream(self, 
        ...
        try:
            ...
            logger.warning("TextIteratorStreamer not available, falling back to non-streaming implementation")
    ...
    def reload_to_gpu(self):
        ...
        """
        if self.device == "cpu" and torch.cuda.is_available() and hasattr(self, "model"):
    ...
    def is_on_gpu(self) -> bool:
        ...
        if hasattr(self.model, "hf_device_map"):
            ...
            return any("cuda" in str(device) for device in self.model.hf_device_map.values())
