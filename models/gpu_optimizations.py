...
def get_device_map(model_size_gb: float, available_gpus: Optional[List[int]] = None) -> Dict[str, Union[int, str]]:
    """
...
def estimate_model_size(model_id: str, num_layers: Optional[int] = None, hidden_size: Optional[int] = None) -> float:
    """
    ...
    if num_layers and hidden_size:
        ...
        params_per_layer = (
            ...
            # Biases
            5 * hidden_size + intermediate_size  # Attention + FFN biases
        )
        
        # Total parameters
        total_params = embedding_params + (params_per_layer * num_layers) + hidden_size * vocab_size
