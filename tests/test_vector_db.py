...
class TestFaissVectorDB(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.num_vectors = 10
        self.test_vectors = np.random.rand(self.num_vectors, self.dimension).astype(
            np.float32
        )
        # Normalize for inner product similarity
        for i in range(self.num_vectors):
            self.test_vectors[i] = self.test_vectors[i] / np.linalg.norm(
                self.test_vectors[i]
            )

        # Create query vectors
        self.num_queries = 3
        self.query_vectors = np.random.rand(self.num_queries, self.dimension).astype(
            np.float32
        )
        # Normalize for inner product similarity
        for i in range(self.num_queries):
            self.query_vectors[i] = self.query_vectors[i] / np.linalg.norm(
    ...
    def test_save_and_load(self):
        ...
        np.testing.assert_array_almost_equal(
            orig_similarities, new_similarities, decimal=5
        )
