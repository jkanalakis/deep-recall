...
class InferenceBenchmark:
    ...
    async def benchmark_latency(
        self, prompts: List[str], batch_size: int = 1, num_runs: int = 10
    ) -> Dict[str, Any]:
        """
            ...
            for _ in range(num_runs):
                ...
                try:
                    ...
                    if response.status_code != 200:
                        logger.error(
                            f"Error during inference: {response.status_code} - {response.text}"
                        )
                        continue
    ...
    async def benchmark_throughput(
        self, prompt: str, duration_seconds: int = 60, max_concurrent: int = 10
    ) -> Dict[str, Any]:
        """
...
def inference_benchmark():
    ...
    api_url = os.environ.get(
        "INFERENCE_API_URL", "http://localhost:8000/v1/completions"
    )
...
async def test_benchmark_latency(inference_benchmark):
    ...
    results = await inference_benchmark.benchmark_latency(
...
async def test_benchmark_throughput(inference_benchmark):
    ...
    results = await inference_benchmark.benchmark_throughput(
        prompt, duration_seconds=10, max_concurrent=3  # Short duration for testing
    )
