...
class TestEmbeddingModels(unittest.TestCase):
    ...
    def test_embedding_model_factory(self):
        ...
        with patch('memory.embeddings.sentence_transformer.SentenceTransformer', MockSentenceTransformer):
            model = EmbeddingModelFactory.create_model("sentence_transformer", model_name="all-MiniLM-L6-v2")
        ...
        with patch('memory.embeddings.transformer.AutoModel'):
            with patch('memory.embeddings.transformer.AutoTokenizer'):
                model = EmbeddingModelFactory.create_model("transformer", model_name="bert-base-uncased")
    ...
    def test_transformer_model(self):
        ...
        with patch('memory.embeddings.transformer.AutoModel.from_pretrained', return_value=mock_model):
            with patch('memory.embeddings.transformer.AutoTokenizer.from_pretrained', return_value=mock_tokenizer):
