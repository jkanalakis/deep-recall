...
class TestSemanticSearch(unittest.TestCase):
    ...
    def test_basic_search(self):
        ...
        results = self.semantic_search.search(query="Animals like foxes and dogs", k=3)
    ...
    def test_pagination(self):
        ...
        page1 = self.semantic_search.search(
            query="programming and technology", k=10, page=1, items_per_page=2
        )

        page2 = self.semantic_search.search(
            query="programming and technology", k=10, page=2, items_per_page=2
        )
        ...
        if len(page1["results"]) > 0 and len(page2["results"]) > 0:
            self.assertNotEqual(page1["results"][0]["id"], page2["results"][0]["id"])

    def test_filtering(self):
        """Test metadata filtering functionality."""
        # Search with metadata filter
        results = self.semantic_search.search(
            query="technology and science", filter_metadata={"user_id": "user_1"}
        )
    ...
    def test_hybrid_search(self):
        ...
        # First verify that the text with "cat" and "mat" exists in the memory store
        cat_mat_text = "The cat sat on the mat and took a nap"
    ...
    def test_different_metrics(self):
        ...
        cosine_results = self.semantic_search.search(
            query="artificial intelligence", metric="cosine"
        )

        euclidean_results = self.semantic_search.search(
            query="artificial intelligence", metric="euclidean"
        )

        dot_results = self.semantic_search.search(
            query="artificial intelligence", metric="dot"
        )
...
class TestMemoryRetrieverWithSemanticSearch(unittest.TestCase):
    ...
    def setUp(self):
        ...
        EmbeddingModelFactory.create_model = (
            lambda model_type, **kwargs: MockEmbeddingModel()
        )
    ...
    def _add_test_data(self):
        ...
        for i, text in enumerate(texts):
            metadata = {"user_id": f"user_{i % 2}", "tags": ["test", f"topic_{i}"]}
    ...
    def test_search_memory(self):
        ...
        results = self.memory_retriever.search_memory(
            query="artificial intelligence and machine learning", k=3, metric="cosine"
        )
    ...
    def test_hybrid_search_memory(self):
        ...
        results = self.memory_retriever.hybrid_search_memory(
            query="programming language Python", k=2
        )
    ...
    def test_search_by_metadata(self):
        ...
        hybrid_results = self.memory_retriever.search_by_metadata(
            filter_metadata={"user_id": "user_0"}, query="artificial intelligence"
        )
