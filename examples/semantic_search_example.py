...
for user_id, texts in conversations.items():
    ...
    for i, text in enumerate(texts):
        ...
        if "machine" in text.lower() or "learning" in text.lower() or "ai" in text.lower():
            metadata["tags"].append("ai")
        elif "tourist" in text.lower() or "visit" in text.lower() or "travel" in text.lower():
            metadata["tags"].append("travel")
        elif "cook" in text.lower() or "recipe" in text.lower() or "food" in text.lower():
...
print(f"Page {advanced_results['pagination']['page']} of {advanced_results['pagination']['total_pages']}")
...
combined_results = memory_retriever.search_by_metadata(
    ...
    query="TensorFlow and deep learning",
    k=2
)
