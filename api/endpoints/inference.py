...
class MemoryServiceClient:
    ...
    async def get_memories(self, query: str, k: int = 3, threshold: Optional[float] = None, 
                         filter_metadata: Optional[Dict[str, Any]] = None, user_id: str = None) -> List[Dict]:
...
class InferenceRequest(BaseModel):
    ...
    use_memory: bool = Field(default=True, description="Whether to include relevant memories")
    memory_k: int = Field(default=3, description="Number of memories to retrieve if use_memory is True")
    model: str = Field(default="default", description="Model identifier to use for inference")
...
class InferenceResponse(BaseModel):
    ...
    memories_used: Optional[List[Dict[str, Any]]] = Field(default=None, description="Memories used for context")
    finish_reason: str = Field(..., description="Reason for completion (e.g., 'stop', 'length')")
    created_at: str = Field(..., description="Timestamp when the response was generated")
