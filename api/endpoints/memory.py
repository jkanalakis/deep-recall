...
class MemoryQueryInput(BaseModel):
    ...
    threshold: Optional[float] = Field(default=None, description="Optional similarity threshold (0-1)")
    filter_metadata: Optional[Dict[str, Any]] = Field(default=None, description="Optional metadata filtering criteria")
...
class MemoryServiceClient:
    ...
    async def add_memory(self, text: str, metadata: Optional[Dict[str, Any]] = None, user_id: str = None) -> MemoryResponse:
    ...
    async def get_memories(self, query: str, k: int = 3, threshold: Optional[float] = None, 
                          filter_metadata: Optional[Dict[str, Any]] = None, user_id: str = None) -> List[MemoryResponse]:
        ...
        for i in range(min(k, 3)):  # Simulate fewer results than requested
            ...
            results.append(
                MemoryResponse(
                    ...
                    metadata={"relevance": 0.9 - (i * 0.1), "user_id": user_id, **filter_metadata},
                    timestamp=timestamp
                )
            )
