...
from api.auth.providers import JWTAuth, TokenData, get_current_user, require_permission
    ...
    try:
        ...
        return UserResponse(
            ...
            roles=[
                ...
                for role_name in [
                    user_repository.roles.get(role_id, Role(id="", name="unknown")).name
                ]
            ],
...
async def create_api_key(
    key_request: ApiKeyRequest, current_user: User = Depends(get_current_user)
):
    """
    ...
    api_key = user_repository.create_api_key(
        user_id=current_user.id, name=key_request.name, expires_at=expires_at
    )
...
async def disable_api_key(
    key_prefix: str = Path(
        ..., min_length=8, description="Prefix of the API key to disable"
    ),
...
@router.delete(
    "/users/{user_id}", dependencies=[Depends(require_permission("user:manage"))]
)
